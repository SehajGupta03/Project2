...
void ViewTimeline::OnPlayPlayFromBeginning(wxCommandEvent& event)
{
    ...
    GetPicture()->GetMachineOne()->SetMachineFrame(0);
    GetPicture()->GetMachineTwo()->SetMachineFrame(0);
...
void ViewTimeline::OnTimer(wxTimerEvent& event)
{
    ...
    auto machine1 = GetPicture()->GetMachineOne();
    auto startFrameOne = machine1->GetStartFrame();
    auto machine2 = GetPicture()->GetMachineTwo();
    auto startFrame = machine2->GetStartFrame();

    if(startFrameOne <= frame)
    {
        machine1->SetMachineFrame(frame - startFrameOne);
    }

    if(startFrame > -1 && startFrame <= frame)
    {
        machine2->SetMachineFrame(frame - startFrame);
    }
}
